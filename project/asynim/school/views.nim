...
proc detailPersonView*(db: DbConn, person, id: string): Future[string] {.async.} =
  ...
  var content: string
  if person == "teacher":
    var item = db.select(Teacher, "\"Teacher\".id = $1" % $id.parseInt)[0]
    ...
    var item = db.select(Student, "\"Student\".id = $1" % $id.parseInt)[0]
    content = card.multiReplace(
      ("{{ title }}", $item),
      ("{{ subtitle }}", item.birthDate.toStr),
...
template detailDirectorView*(): untyped =
  let card = await getFileData("templates/card.html")
  var item = dbSchool.select(Director, "\"Director\".id = $1" % $1)[0]
  await getContent(
    title("Директор"),
    h1(
...
template updateDirectorView*(): untyped =
  ...
  if dbSchool.exists(Director, "\"Director\".id = $1" % $1):
...
template detailDirectorApiView*() =
  var content: JsonNode = newJObject()
  let item = dbSchool.select(Director, "\"Director\".id = $1" % $1)[0]
  content = %* item
...
template detailPersonApiView*(person, personId: string) = 
  var content: JsonNode = newJObject()
  if person == "teacher":
    let item = dbSchool.select(Teacher, "\"Teacher\".id = $1" % $personId.parseInt)[0]
    ...
    let item = dbSchool.select(Student, "\"Student\".id = $1" % $personId.parseInt)[0]
    content = %* item
    content["birthDate"] = %* item.birthDate.toStr 
  resp content